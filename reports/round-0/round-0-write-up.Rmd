...
- In three of four scenarios, models projected repeated resurgence of COVID-19 burden across Europe continuing over the next twelve months. 
...
In the pilot round we adopted identical scenarios to the US Scenario Hub Round 13, outlined below. The scenarios were deemed to have policy relevance in Europe, with additional benefits from using the same scenario as the US hub both in team engagement (being inclusive to teams already submitting to the US hub) and understanding any potenetial differences between the situation in Europe and the US.
...
Models projecting the same scenario in the same country showed striking differences in the expected dynamics. It is different to pinpoint this to individual model choices given the small number of scenario-country combinations for which this is the case, especially as in this round we did not have access to the trajectories from individual model runs (this was subsequently changed for future iterations). That said, it was noticeable that the `ECDC-CM_ONE` model, which modelled waning of immunity as exponential decline generally produced smooth and not very peaked trajectories. Other models, such as the `USC-SIkJalpha` and `ICM-agentModel` models, both of which assumed that vaccine immunity stayed constant for 2 and 3 months, respetively, before waning commenced, produced more wave-like and peaked behaviour, especially with the introduction of new variants but to some degree even without them. This suggests that the precise shape of immune waning can have a strong impact on the expected burden, with implications for healthcare usage (constantly at a high level vs. in sharp bursts) and societal consequences of the level of disease at any given time.
...
Future scenario submissions will be based on samples which will permit additional analyses, such as the number, height and width of wave peaks, cumulative burden over given periods, or the number of weeks during which a certain incidence threshold is exceeded.
